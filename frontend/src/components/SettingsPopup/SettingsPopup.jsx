...
import {setUserChannel, setUserData} from '../../redux/slices/userDataSlice'
...
const SettingsPopup = ({isOpen, setIsOpen}) => {
	...
	const [value, setValue] = useState(userChannel.name)
	...
	const changeLogin = (e) => {
		...
		try {
			console.log('zapros')
			const request = doRequest('/api/v1/channel/me/', userToken, "PATCH", obj)
				...
				<form method="post" className={s.wrapper__info}>
					<div className={s.info__block}>
						<label className={s.info__label__content}>
							<h2 className={s.info__title__content}>название канала</h2>
